...
\begin{figure}[h]
  \centering
    \includegraphics[width=0.75\textwidth]{Imagenes/sisAplication.PNG}
    \caption{Aplicación de la regla $\phi_{SIS}^t(x)$}
\end{figure}

\begin{figure}[h]
  \centering
    \includegraphics[width=0.75\textwidth]{Imagenes/sirAplication.PNG}
...
\underline{\textit{Observación:}} Es importante recordar que el paso del estado $I$ al estado $S$ (o al $R$) no depende del sistema de vecindades, ya que se asume que no es necesario el contacto con células no infectadas para que una que se encuentra enferma se recupere. \\
...
\begin{figure}[h]
  \centering
    \includegraphics[width=0.9\textwidth]{Imagenes/sisEn30.PNG}
...
\begin{figure}[h]
  \centering
    \includegraphics[width=0.9\textwidth]{Imagenes/sirEn30.PNG}
...
En la figura (\ref{fig:sirEn30}) podremos visualizar las soluciones obtenidas al analizar el mismo escenario con los modelos clásicos (izquierda) y las generadas por nuestras reglas de evolución $\phi_{SIS}^t(x)$ y $\phi_{SIR}^t(x)$ (derecha). Para generar las gráficas de las reglas SIS y SIR definidas durante esta sección, utilizaremos el siguiente script:
