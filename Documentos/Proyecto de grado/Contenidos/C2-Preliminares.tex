...
Para determinar los escenarios bajo los cuales una enfermedad es endémica debemos calcular el valor de $R_0$ para nuestro sistema de ecuaciones diferenciales (\ref{eq:modeloSIS}). Recuerde que para determinar el valor de $R_0$ se considera una población completamente susceptible, es decir, $S=1$ e $I=0$.
...
Sea $X$ un conjunto cualquiera. La familia de todos los subconjuntos de $X$ es una topología sobre $X$ y es llamada la \textit{topología discreta} de $X$.
\end{example}

\begin{example}\label{ex:exBase}
Considere al conjunto $X=\{a,b,c\}$ y a una subcolección $\mathcal{A}$ de $X$ definida como: 
...
La colección de conjuntos unipuntuales de cualquier conjunto $X$ forma una base para la topología discreta de $X$.
\end{example}
...
En el ejemplo \ref{ex:exBase} podremos identificar al conjunto $\mathcal{V}(a)$ como:
...
Considere la topología $\mathcal{A}$ y el espacio $X$ del ejemplo \ref{ex:exBase}. Las vecindades minimales para este caso son:
...
\begin{figure}[h]\label{fig:AC a matriz}
  \centering
...
\begin{figure}[h]\label{fig:Fuego Netlogo}
  \centering
...
\begin{figure}[h]\label{fig:Moore - Von neumann}
  \centering
...
\begin{itemize}
    ...
    \begin{figure}[h]\label{fig:Regla30}
      \centering
