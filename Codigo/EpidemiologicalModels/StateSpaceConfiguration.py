...
class createSpace:
    ...
    def __init__(self, system = None, nRows = 0, nColumns = 0):
        ...
        self.system = system
        self.validateSystemConditions()

    def validateSystemConditions(self):
        if self.nRows == 0 and self.nColumns == 0 and self.system == None:
            print("Debe agregar una configuración para la el sistema (filas/columnas o sistema creado)")
        elif self.nRows != 0 and self.nColumns != 0 and self.system == None:
            self.system = np.zeros((self.nRows, self.nColumns))
        elif self.system.any != None:
            self.nRows, self.nColumns = self.system.shape
